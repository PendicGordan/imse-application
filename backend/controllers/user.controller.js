...
const create = async function(req, res){
    ...
    bcrypt
        ...
        .then(newUser => {
            ...
            return ReS(res, {token: newUser.getJWT(newUser.dataValues.id, newUser.dataValues.email)});
        })
        .catch(err => {
            console.log(err);
        });
...
const resetPW = async function (req, res){
    ...
    if(sendResetEmail(req.body.email, user.dataValues.hash, user.getJWT(user.dataValues.id, user.dataValues.email)))
